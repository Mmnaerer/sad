...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["IWasHuman", "Line", "3.0"];
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && !mem.includes(client.user.jid)) {
        ...
        buttons = [
          { buttonId: `y`, buttonText: { displayText: "Hai Member Baru 👋" }, type: 1 },
        ];
        imageMsg = (
      ...
      if (anu.action == "demote") {
        ...
        let buffer = await getBuffer(
          `https://api-yogipw.herokuapp.com/api/demote?name=${anu_user}&msg=Turut%20Berduka%20cita%20telah%20di%20demote&mem=${groupAdmins.length}&picurl=${ppimg}&bgurl=https://cdn.discordapp.com/attachments/819995259261288475/835055559941292032/style.jpg`
        );

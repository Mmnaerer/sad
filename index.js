...
thumb = fs.readFileSync("./stik/banner_menu.png");
numbernye = "0";
waktu = "-";
alasan = "-";
autojoin = false;
...
module.exports = client = async (client, mek) => {
  try {
    ...
    const ownerNumber = '6285955372133'
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "6285859202162@s.whatsapp.net";
    ...
    if (isGroup && isAntilink && !mek.key.fromMe) {
      if (budy.includes("://chat.whatsapp.com/")) {
        ...
        reply("ANTILINK DETECTED!! OLEH KARENA ITU SAYA KICK");
    ...
    if (isGroup && isAntivirtex && !mek.key.fromMe) {
      if (budy.length > 700) {
        if (isGroupAdmins) return reply("admin bebas");
        reply("ANTIVIRTEX DETECTED!! OLEH KARENA ITU SAYA KICK");
        client.groupRemove(from, sender);
      }
    }
...
`;
        sendButImage(from, menu, "© Creator By IWasHuman\nJangan Lupa Subrek ya bang :)", thumb, [
          {
            buttonId: `${prefix}youtubec`,
            buttonText: {
              displayText: `Youtube 🎥`,
            },
            type: 1,
        ...
        break;
      case "youtubec":
      reply('Jangan Lupa di Subscribe')
      await reply('https://youtube.com/c/IWasHuman')
      break;
      case "owner":
        sendKontak(from, owner, "IWasHuman", "Sibuk");
        break;
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`;
      ...
      case "term":
        ...
        break;
      case "join":
      if (!isOwner) return client(`Hanya Owner Yang bisa memakai fitur ini!`)
        try {
